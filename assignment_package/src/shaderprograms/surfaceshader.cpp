...
SurfaceShader::SurfaceShader(OpenGLContext *context)
    : ShaderProgram(context),
      ...
      unifModel(-1), unifModelInvTr(-1), unifView(-1), unifProj(-1),unicamera(-1)
{}
...
void SurfaceShader::setupMemberVars()
{
    ...
    //assign unicamera with a handle
    unicamera = context->glGetUniformLocation(prog,"u_Camera");
...
void SurfaceShader::SetCameraPosition(const glm::vec3& pos){
    useMe();
    if(unicamera != -1){
        context->glUniform3f(unicamera,pos.x,pos.y,pos.z);
    }
}

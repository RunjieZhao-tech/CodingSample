...
void main()
{
	...
	float res = max(pow(dot(normalize((fs_CameraPos - fs_Pos + fs_LightVec) / 2),normalize(fs_Nor)),30),0);
    float lightIntensity = diffuseTerm + ambientTerm +res;   //Add a small float value to the color multiplier
